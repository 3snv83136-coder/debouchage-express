...
const s = StyleSheet.create({
  ...
  validite: { marginTop: 18, padding: 8, backgroundColor: "#FFF7ED", borderRadius: 4, fontSize: 9, color: "#9a3412" },
  signBox: { marginTop: 22, flexDirection: "row", justifyContent: "space-between" },
  signCol: { width: 220, borderTop: "1 solid #ccc", paddingTop: 6, fontSize: 8, color: "#444" },
  pay: { marginTop: 18, fontSize: 8, color: "#444" },
      ...
      <Page size="A4" style={s.page}>
        <View style={s.top}>
          ...
          <View style={s.factBox}>
            <Text style={s.factTitle}>DEVIS</Text>
            <Text style={s.meta}>N° {d.numero}</Text>
            <Text style={s.meta}>Date : {d.date}</Text>
            <Text style={s.meta}>Valable jusqu'au : {d.valideJusquau}</Text>
          </View>
        </View>
        ...
        <Text style={s.validite}>
          Devis gratuit et sans engagement. Prix valables jusqu'au {d.valideJusquau}. Les travaux ne débuteront qu'après acceptation du présent devis.
        </Text>

        <View style={s.signBox}>
          <View style={s.signCol}>
            <Text>Date et signature du client</Text>
            <Text style={{ marginTop: 2 }}>(précédées de la mention « Bon pour accord »)</Text>
          </View>
          <View style={s.signCol}>
            <Text>{d.societe.raisonSociale}</Text>
          </View>
        </View>

        <Text style={s.pay}>
          Coordonnées bancaires (acompte éventuel) : IBAN {d.societe.iban}.
        </Text>
